...
// Y = A-B
void *MtrxSub(void *Y, void *A, void *B)
{
	int elements, i;
	Vector y, a, b;
 ...
 *
 ***********************************************************************/
Matrix MtrxInvSVD(Matrix Y, Matrix A)
{
	int m,n,i;
...
Matrix MtrxInv(Matrix Y, Matrix A)
{
	...
	if (MtrxEqualSizes(Y,A))
	{
		...
		if (m==n)
		{
			// Allocate a matrix B of "solutions" and copy A into Y
			B = MtrxZeros(m, 1);
			MtrxCopyContents(Y, A);

			// Invoke Gauss-Jordan solver
			result = MtrxGaussJ(Y, B);
...
/***************************************************************************
*	  FUNCTION		 : GaussJ	
*     TIME and DATE  : 19:00    06.02.96
*     ABSTRACT       : Linear equation solution by Gauss-Jordan elimination.
*                      a[1..n][1..n] is the input matrix. b[1..n][1..m] is
*                      input containing the m right-hand side vectors in
*                      the linear equation set a*x = b.
*                      On output a is replaced by its inverse, and b is
*                      replaced by the corresponding set of solution vectors
*
*     PARAMETERS    :
*     a             : double**   - I/O  : square matrix a (in) and inv(a) (out)
*     b             : double*    - I/O  : input vectors (in) and solutions (out)
*
*     RETURNS       : void
*
*     NOTES          : Numerical recipes in C, pp 36-41
*                      (C) Copr. 1986-92 Numerical Recipes Software <6!>3+1'.
*
*     AUTHOR         : Kjetil R›e, ABB Industri AS
*	  HISTORY		 : Kristoffer Kvam, 06.07.98: Adaptment to Mtrxlib.h
*****************************************************************************/
...
//****************************************************************************
//	These lines are added to avoid dynamic allocation during execution of
//	the main loop
//****************************************************************************	
	int indxc[100];
	int indxr[100];
	...
	int n = MtrxGetM(a);	// Dimension of a
	int m = MtrxGetN(b);    // Number of equations to solve

//****************************************************************************
//	These lines are removed to avoid dynamic allocation during execution of
//	the main loop
//****************************************************************************
//    indxc=ivector(1,n);
//	indxr=ivector(1,n);
...
//			free_ivector(indxc,1,n);
		...
		pivinv=(double) 1.0/a[icol][icol];
		a[icol][icol]=(double) 1.0;
		for (l=1;l<=n;l++) a[icol][l] *= pivinv;
		for (l=1;l<=m;l++) b[icol][l] *= pivinv;
		for (ll=1;ll<=n;ll++)
			if (ll != icol) {
				dum=a[ll][icol];
				a[ll][icol]=(double) 0.0;
