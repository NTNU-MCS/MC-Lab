...
#include <windows.h>
#define ENTERCRITICALSECTION EnterCriticalSection (&CriticalSection);
#define LEAVECRITICALSECTION LeaveCriticalSection (&CriticalSection);
...
void *mmalloc(unsigned int nbytes)
{
	...
	if ((prevp = freep) == NULL) /* no free list yet */
	{
		...
		base = (Header*)buffer;  /* Instead of NT system call */
...
int mmemleak()	/* Returns  */
{
	...
	if (freep)
	{
		bytes = sizeof(Header)*(base->s.ptr->s.size + 2);
...
long mbytesfree()
{
	...
	if (freep)	// Has mmalloc been run, at least once?
	{
