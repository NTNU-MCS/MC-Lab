...
double kpdiv(double num, double den)
{
	double y;

	y = num/den;
/*
	if (fabs(den) > MTRXEPSILON)
		y = num/den;
	else
	{
		y = 1.0;
...
void MtrxSvdComp (Matrix U, Matrix S, Matrix V, Matrix A)
{
	...
	for (i=n;i>=1;i--)
	{
		if (i<n)
		{
			if (g!=0.0)
			{
				for (j=l;j<=n;j++)
					// V[j][i] = (U[i][j]/U[i][l]) / g;
					V[j][i] = kpdiv( kpdiv(U[i][j], U[i][l]), g);
				for (j=l;j<=n;j++)
				{
					s = 0.0;
					for (k=l;k<=n;k++)
	...
	for (k=n;k>=1;k--)
	{
		for (its=1;its<=MTRXMAXITERATIONS;its++)
		{
			...
			f = kpdiv( ((y - z)*(y + z) + (g-h)*(g+h)) , 2.0*h*y);
			g  = pythag(f, 1.0);
			// f  = ((x - z)*(x + z) + h*((y/(f + sign(g,f))) - h))/x;
			f = kpdiv(((x - z)*(x + z) + h*(( kpdiv(y, (f + sign(g,f))) ) - h)), x);
			c  = 1.0;
			s  = 1.0;
			for (j=l;j<=nm;j++)
			{
