 ...
 *
 * Rev.1 - 20150116 Adding Server functionality over TCP/IP. Author: Torgeir Wahl, NTNU
 *					
 */
...
void sendData( int sockfd, int *x, char *y ) {
  int n, i, xbool, buflen;
  char buffer[256];

  sprintf( buffer, "%10d\t%10d\t%10d\t%10d\t%10d\t%10d\t", x[0], x[1], x[2], x[3], x[12], x[13] );
  ...
  for(i =0; i < 16; i++) {
	  if(y[i] == 0)
		  ...
		  xbool = 32767;
	  sprintf(&buffer[buflen + (i * 11)], "%10d\t", xbool);
  }
...
int main() {
	...
	printf("\n\nJoystick C/S Controller. Version: TWa20150116\n\n");
...
/*************************************************************************
			...
			for( x=5 ; x<num_of_axis ; ++x )
				printf("A%d: %5d  ", x, axis[x] );
